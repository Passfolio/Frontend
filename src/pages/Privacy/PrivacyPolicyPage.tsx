    ...
    >
      ...
      <main className="relative z-[1] mx-auto w-full max-w-[720px] flex-1 px-4 pb-20 pt-24 md:px-6 md:pt-28">
        <header className="mb-10 md:mb-12">
          ...
          <h1 className="mt-2 text-3xl font-semibold tracking-tight text-white md:text-4xl">개인정보 처리방침</h1>
          <p className="mt-3 text-sm leading-relaxed text-zinc-400">
            Passfolio 서비스 이용 시 개인정보의 수집·이용·보관·파기 등에 관한 안내입니다.
          </p>
        ...
        <div className="flex flex-col gap-10">
          {PRIVACY_SECTION_LIST.map((section) => (
            <section key={section.id} aria-labelledby={`privacy-${section.id}`}>
              <h2 id={`privacy-${section.id}`} className="text-lg font-semibold tracking-tight text-white md:text-xl">
                {section.title}
              </h2>
