...
export type ProfileMenuItem = (typeof MENU_LIST)[number];

/** `/profile?section=` 쿼리와 사이드바 메뉴 라벨 매핑 */
export const MENU_SECTION_SLUG: Record<ProfileMenuItem, string> = {
    프로필: 'profile',
...
export const SECTION_SLUG_TO_MENU = Object.fromEntries(
    (Object.entries(MENU_SECTION_SLUG) as [ProfileMenuItem, string][]).map(([label, slug]) => [slug, label]),
