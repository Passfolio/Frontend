...
import { fetchGitHubRepos, type GitHubRepoItemType, type GitHubRepoType } from '@/api/GitHub/githubApi';
import { useAuth } from '@/context/Auth/AuthContext';

export type RepositoryColumnConfigType = {
    chipLabel: string;
    type: GitHubRepoType;
};

const resolveGitHubRepoWebUrl = (
    repo: GitHubRepoItemType,
            ...
            >
                <div className="flex flex-col gap-2.5">
                    {repos.map((repo, index) => {
                        ...
                        return (
                            <article key={key} className={cardClass} style={cardStyle}>
                                <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-white/15 to-transparent opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
                                {cardContent}
                            </article>
                        );
