...
export function educationFingerprint(history: EducationHistoryItem[]): string {
  return history
    .map((e) => [e.name, e.type, e.region, e.department, e.degree, e.duration].join('\u001f'))
    .join('\u001e');
}

export function loadStoredDevSpecIds(userId: string | number): StoredDevSpecIds | null {
  try {
    const raw = localStorage.getItem(storageKey(userId));
    if (!raw) return null;
    return JSON.parse(raw) as StoredDevSpecIds;
  } catch {
    return null;
  }
}

export function saveStoredDevSpecIds(userId: string | number, payload: StoredDevSpecIds): void {
